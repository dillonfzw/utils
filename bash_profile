...
do
  ...
  if [ -z "$val" -o -z "$(locale -a 2>/dev/null | grep -Fx "$val")" ] && \
    ...
    echo "[I]: Change $item from \"$val\" to \""`eval "echo \\\$$item"`"\""
  fi
done
...
if [ -n "$TMUX" -a -z "$CSCOPE_EDITOR" ]; then
  ...
  echo "[I]: Define CSCOPE_EDITOR to $CSCOPE_EDITOR"
fi
...
if [ "$ACTIVATE_PYTHON_VIRTUALENV" != "no" -a "`type -t lsvirtualenv`" != "function" ]; then
  if if command -v virtualenvwrapper.sh >/dev/null 2>&1; then
    ...
    echo "[E]: Fail to source virtualenvwrapper.sh with WORKON_HOME equals to $WORKON_HOME"
  fi
fi
...
if [ -n "$fbcreg" -a -f "$fbcreg" ]; then
    ...
    if [ $sdiff -gt $((15 * 24 * 3600)) ]; then
        echo "[I]: Reset BCompare trial data"
...
if ! command -v lscm >/dev/null 2>&1; then
  ...
  if [ -x $RTC_SCMTOOLS_HOME/$RPATH/lscm ]; then
    ...
    echo "[I]: Add RTC-scmTools to PATH as $RTC_SCMTOOLS_HOME/$RPATH"
  fi
fi
unset RPATH
...
if [ -n "$_CONDA_PATH_Chae4dok9e" ]; then
  ...
  if [ \( "$_CONDA_VER_Chae4dok9e" != "$PYVER" \) -o \
    ...
    echo "[I]: Remove $_CONDA_PATH_Chae4dok9e from PATH"
...
if true || [ -z "$_CONDA_PATH_Chae4dok9e" ]; then
  ...
  do
    if [ -d $item ]; then
      ...
      if [ $? -eq 0 ]; then
        echo "[I]: Eval $item/bin/conda shell.bash hook..."
        eval "$__conda_setup"
        ...
        if [ -f "${item}/etc/profile.d/conda.sh" ]; then
          echo "[I]: Source $item/etc/profile.d/conda.sh..."
          . "${item}/etc/profile.d/conda.sh"
        else
          echo "[I]: Append $item/bin to PATH"
